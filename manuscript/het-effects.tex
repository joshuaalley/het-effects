...
Assume that the outcome variable ${y}$ is normally distributed with mean $\mu_i$ and standard deviation $\sigma$.\footnote{Researchers should use binary, categorical and other outcome likelihoods and adjust for autocorrelation and clustering as needed.}
...
Priors also help regularize estimates by pulling extreme groups towards the overall mean.\footnote{The random intercepts $\alpha_g$ and varying slopes $\theta_g$ should usually have a common multivariate normal prior to capture correlations between them.}
...
Hierarchical models like the above are unidentified with one modifier, and the extra information they provide is less valuable. 


% two modifiers
With two or more modifiers, hierarchical models will add value beyond interactions. 
...
The thresholds where the number of modifiers becomes an issue for hierarchical modeling depends on the data. 
