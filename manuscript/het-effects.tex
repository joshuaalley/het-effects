...
This paper describes why, when, and how to use a type of hierarchical model to encompass multiple sources of heterogeneity and regularize estimates.
...
The downside is that strong regularization in this model breaks down if there are many small groups.
...
Social scientists often propose conditional theories \citep{ClarkGolder2023} and are interested in how different units respond to the same stimulus for normative or policy reasons.
But while most theories consider a single modifier, many theories proposing distinct modifiers for the same independent variable suggest that multiple modifiers are usually present.
...
The share of quantitative papers that estimate heterogeneous effects has risen steadily so that in 2024, almost 70\% of replication files contain at least one heterogeneous effects estimate. 
...
Researchers might instead fit separate pairwise interactions for each modifier, but that leads to more models and multiple testing problems.
...
This is one reason why these techniques are far less popular.
...
Second, they should take that model of treatment effects and connect it to a model linking treatment effects and the outcome.
...
While frequentist estimation of hierarchical models is possible, Bayesian estimation is easy, more informative and potentially more robust \citep{Stegmueller2013}.
...
I then employ a simulation study to compare OLS and hierarchical estimates of treatment effects under different conditions.
...
Putting priors on the $\alpha$, $\beta$, and $\gamma$ parameters will only matter in very small datasets.
...
Fitting splines or smooth functions is also possible, or researchers might extend the model in continuous space with Gaussian process regression.
...
A third simulation adjusts the data-generating process behind the heterogeneity to clarify the trade offs around partial pooling.
...
I use root mean squared error because the hierarchical model's shrinkage will introduce bias but compensate with lower variance \citep{CliffordRainey2024}.
...
In a third simulation, I examine the consequences of hierarchical models what the true distribution of effects includes significant outliers. 
...
As the degrees of freedom in the student-t distribution drop, outliers become more common. 
...
While the hierarchical model still outperforms OLS in terms of overall accuracy and RMSE, the bias is worse for outlier effects.
...
First, researchers should consider using student-t priors on effects when they suspect potential outliers, and compare normal or student-t priors in terms of model fit. 
...
To start, I plot the two regressions from the hierarchical model. 
...
As conditional theories proliferate in political science scholarship, many independent variables have multiple potential modifiers. 
Interaction models cannot easily handle this shift, as adding three or more variables to an interaction reduces the interpretation benefits, but fitting separate models for each modifier increases the risk of spurious results. 
...
These tasks would further expand the benefits of hierarchical modeling in tackling multiple conditional arguments in a single framework.
